...
describe("Bath Token contract", function () {
  ...
  async function deployTokenFixture() {
    ...
    const FarmContract = await ethers.getContractFactory("GenesisFarm");
    const [owner, addr1, addr2] = await ethers.getSigners();

    // To deploy our contract, we just have to call Token.deploy() and await
    // its deployed() method, which happens once its transaction has been
    // mined.
    const genesisFarm = await FarmContract.deploy();

    await genesisFarm.deployed();

    // Fixtures can return anything you consider useful for your tests
    return { FarmContract, genesisFarm, owner, addr1, addr2 };
  }
  ...
  describe("Deployment", function () {
    ...
    it("Should set the right owner", async function () {
      ...
      // things went well
      const { FarmContract, owner } = await loadFixture(deployTokenFixture);
      ...
      expect(await FarmContract.owner()).to.equal(owner.address);
    });
  });
