...
contract Bath is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 BATH-WFTM LP -> BATH pool
    uint256 public constant INITIAL_BATH_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BATH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) ERC20("BATH", "BATH") {
        // Mints 1 BATH to contract creator for initial pool setup
    ...
    function setTaxTiersTwap(uint8 _index, uint256 _value) public onlyTaxOffice returns (bool) {
        ...
        if (_index < getTaxTiersTwapsCount().sub(1,"getTaxTiersTwapsCount bug")) {
            require(_value < taxTiersTwaps[_index + 1]);
        }
    ...
    function _getBathPrice() internal view returns (uint256 _bathPrice) {
        try IOracle(bathOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bath: failed to fetch BATH price from Oracle");
        }
    }
    ...
    function setBathOracle(address _bathOracle) public onlyOperatorOrTaxOffice {
        require(_bathOracle != address(0), "oracle address cannot be 0 address");
        bathOracle = _bathOracle;
    }
    ...
    /**
     * @notice Operator mints BATH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BATH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBathPrice = _getBathPrice();
            currentTaxRate = _updateTaxRate(currentBathPrice);
            if (currentBathPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function _transferWithTax(
        ...
        uint256 amountAfterTax = amount.sub(taxAmount, "amountAfterTax bug");
        if(burnTax) {
            // Burn tax
    ...
    function distributeReward(
        ...
        require(_bathPool != address(0), "!_bathPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_bathPool, INITIAL_BATH_POOL_DISTRIBUTION);
